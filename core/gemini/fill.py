...
_INSTRUCTIONS = (
    ...
    "5. Before finalizing each item, check your own numbers against the "
    "Atwater rule: kcal should be within about 15% of 4*protein + 4*carbs "
    "+ 9*fat (all in grams per 100g). This is basic physics, not a "
    "guideline -- if your first-pass estimate doesn't reconcile, don't "
    "just report it anyway; adjust whichever of the four numbers you're "
    "least confident in until they're consistent with each other, the "
    "same way a real nutrition label always is. Fields that came from the "
    "API (ground truth) should stay fixed while you adjust the ones you "
    "estimated.\n\n"
